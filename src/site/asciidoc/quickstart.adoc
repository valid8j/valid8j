...
To verify a value with `valid8j`, you can call an entry point method of `valid8j` with your value and a predicate to be checked.
...
In order to make `valid8j` print such a message on a failure, you need to give a "printable" predicate, which we will be describing here.

==== Building a "Printable" Predicate

`pcond`, which powers `valid8j`, offers a set of method to create a printable predicate in `Predicates` class.
...
- `xmllint` needs to be installed in advance.

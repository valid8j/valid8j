  ...
  private static TestSuite<String> stringTestSuite_3() {
    return new TestSuite<>(
        asList(
            ...
            (String v) -> Statement.stringValue(v).split(":").then().containingMatchingElementsInOrder(asList(Predicates.isEqualTo("A"), Predicates.isEqualTo("B"), Predicates.isEqualTo("C"))).done()),
        singletonList("A:B:C"),
        asList("A:B", null));
  }

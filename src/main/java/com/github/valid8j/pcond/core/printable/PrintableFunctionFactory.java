...
import com.github.valid8j.pcond.experimentals.currying.CurryingUtils;
...
import static com.github.valid8j.pcond.core.refl.ReflUtils.formatMethodName;
import static com.github.valid8j.pcond.core.refl.ReflUtils.invokeStaticMethod;
...
public enum PrintableFunctionFactory {
  ...
  public static <R> MultiFunction<R> multifunction(Method method, List<Integer> paramOrder) {
    InternalChecks.validateParamOrderList(paramOrder, method.getParameterCount());
    InternalChecks.requireStaticMethod(method);
    return new MultiFunction.Builder<R>(args -> invokeStaticMethod(method, (paramOrder).stream().map(args::get).toArray()))
        ...
        .identityArgs(asList(method, InternalChecks.validateParamOrderList(paramOrder, method.getParameterCount())))
        .$();
  }
